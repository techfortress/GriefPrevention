...
class RestoreNatureProcessingTask implements Runnable
{
	...
	private ArrayList<Material> leaves; //is used in many situations
	private ArrayList<Material> logs; //is used in many situations
		...
		for(int x = 1; x < snapshots.length - 1; x++)
		{
			for(int z = 1; z < snapshots[0][0].length - 1; z++)
			{
				for(int y = this.seaLevel - 1; y < snapshots[0].length; y++)
				{
					...
					if (this.leaves == null) {
						this.leaves = new ArrayList<Material>();
					...
					if(this.leaves.contains(block.typeId) && (block.data & 0x4) != 0)
					{
		...
		for(int x = 1; x < snapshots.length - 1; x++)
		{
			for(int z = 1; z < snapshots[0][0].length - 1; z++)
			{
				...
				while(thisy > this.seaLevel - 1 && (this.snapshots[x][thisy][z].typeId == Material.STONE || this.snapshots[x][thisy][z].typeId == Material.SANDSTONE))
				{
					...
					if(leftBlock.typeId != Material.AIR && !this.leaves.contains(leftBlock.typeId) && leftBlock.typeId != Material.VINE)
					{
						adjacentBlockCount++;
					}
					if(rightBlock.typeId != Material.AIR && !this.leaves.contains(rightBlock.typeId) && rightBlock.typeId != Material.VINE)
					{
						adjacentBlockCount++;
					}
					if(downBlock.typeId != Material.AIR && !this.leaves.contains(downBlock.typeId) && downBlock.typeId != Material.VINE)
					{
						adjacentBlockCount++;
					}
					if(upBlock.typeId != Material.AIR && !this.leaves.contains(upBlock.typeId) && upBlock.typeId != Material.VINE)
					{
						adjacentBlockCount++;
					}
		...
		for(int x = 1; x < snapshots.length - 1; x++)
		{
			for(int z = 1; z < snapshots[0][0].length - 1; z++)
			{
				for(int y = this.seaLevel - 1; y < snapshots[0].length; y++)
				{
					...
					if (this.logs == null) {
						this.logs = new ArrayList<Material>();
					...
					//skip non-logs
					if(this.logs.contains(block.typeId)) continue;
					...
					if(this.logs.contains(leftBlock.typeId) || this.logs.contains(rightBlock.typeId) || this.logs.contains(upBlock.typeId) || this.logs.contains(downBlock.typeId))
					{
		...
		for(int x = 1; x < snapshots.length - 1; x++)
		{
			for(int z = 1; z < snapshots[0][0].length - 1; z++)
			{
				for(int y = miny; y < snapshots[0].length - 1; y++)
				{
					...
					if(underBlock.typeId == Material.AIR || underBlock.typeId == Material.WATER || underBlock.typeId == Material.LAVA || this.leaves.contains(underBlock.typeId))
					{
			...
			for(int y = Math.max(this.seaLevel - 10, 0); y <= this.seaLevel; y++)
			{
				for(int x = 1; x < snapshots.length - 1; x++)
				{
					for(int z = 1; z < snapshots[0][0].length - 1; z++)
					{
						...
						if(block.typeId == Material.AIR || (block.typeId == Material.WATER && block.data != 0))
						{
							BlockSnapshot leftBlock = this.snapshots[x + 1][y][z];
							BlockSnapshot rightBlock = this.snapshots[x - 1][y][z];
							BlockSnapshot upBlock = this.snapshots[x][y][z + 1];
							...
							if(underBlock.typeId != Material.WATER || underBlock.data != 0) continue;

							//count adjacent source water blocks
							byte adjacentSourceWaterCount = 0;
							if(leftBlock.typeId == Material.WATER && leftBlock.data == 0)
							{
								adjacentSourceWaterCount++;
							}
							if(rightBlock.typeId == Material.WATER && rightBlock.data == 0)
							{
								adjacentSourceWaterCount++;
							}
							if(upBlock.typeId == Material.WATER && upBlock.data == 0)
							{
								adjacentSourceWaterCount++;
							}
							if(downBlock.typeId == Material.WATER && downBlock.data == 0)
							{
								adjacentSourceWaterCount++;
							}
	...
	private int highestY(int x, int z, boolean ignoreLeaves)
	{
		...
		for(y = snapshots[0].length - 1; y > 0; y--)
		{
			...
			if(block.typeId != Material.AIR &&
					...
					!(ignoreLeaves && this.leaves.contains(block.typeId)) &&
					!(block.typeId == Material.FLOWING_WATER) &&
					!(block.typeId == Material.WATER) &&
					!(block.typeId == Material.LAVA) &&
					!(block.typeId == Material.FLOWING_LAVA))
			{
				return y;
			}
		}
	...
	static ArrayList<Material> getPlayerBlocks(Environment environment, Biome biome)
	{
		...
		playerBlocks.add(Material.REDSTONE_WALL_TORCH);
		...
		playerBlocks.add(Material.SKELETON_WALL_SKULL);
		playerBlocks.add(Material.WITHER_SKELETON_SKULL);
		playerBlocks.add(Material.WITHER_SKELETON_WALL_SKULL);
