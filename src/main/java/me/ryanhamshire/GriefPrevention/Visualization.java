...
public class Visualization {
	...
	// reverts a visualization by sending another block change list, this time with
	// the real world block values
	...
	// visualizationType determines the style (gold blocks, silver, red, diamond,
	// etc)
	public static Visualization FromClaim(Claim claim, int height, VisualizationType visualizationType, Location locality) {
		...
		// add top level last so that it takes precedence (it shows on top when the
		// child claim boundaries overlap with its boundaries)
	...
	// handy for combining several visualizations together, as when visualization a
	// top level claim with several subdivisions inside
	// locality is a performance consideration. only create visualization blocks for
	// around 100 blocks of the locality
	private void addClaimElements(Claim claim, int height, VisualizationType visualizationType, Location locality) {
		...
		// set Y values and real block information for any remaining visualization
		// blocks
	...
	// finds a block the player can probably see. this is how visualizations "cling"
	// to the ground or ceiling
	...
	// helper method for above. allows visualization blocks to sit underneath partly
	// transparent blocks like grass and fence
