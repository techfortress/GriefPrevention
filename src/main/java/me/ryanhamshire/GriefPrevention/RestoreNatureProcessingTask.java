...
class RestoreNatureProcessingTask implements Runnable {
	...
	private ArrayList<Material> playerBlocks; // a "complete" list of player-placed blocks. MUST BE MAINTAINED as
												// patches introduce more

	public RestoreNatureProcessingTask(BlockSnapshot[][][] snapshots, int miny, Environment environment, Biome biome,
			Location lesserBoundaryCorner, Location greaterBoundaryCorner, int seaLevel, boolean aggressiveMode,
			boolean creativeMode, Player player) {
		...
		// in aggressive or creative world mode, also treat these blocks as user placed,
		// to be removed
		// this is helpful in the few cases where griefers intentionally use natural
		// blocks to grief,
		// like a single-block tower of iron ore or a giant penis constructed with
		// melons
	...
	public void run() {
		...
		RestoreNatureExecutionTask task = new RestoreNatureExecutionTask(this.snapshots, this.miny,
				this.lesserBoundaryCorner, this.greaterBoundaryCorner, this.player);
		...
		for (int x = 1; x < snapshots.length - 1; x++) {
			for (int z = 1; z < snapshots[0][0].length - 1; z++) {
				for (int y = snapshots[0].length - 2; y > miny; y--) {
					if (snapshots[x][y][z].typeId != Material.SANDSTONE)
						continue;
					...
					if (aboveBlock.typeId == Material.SAND && underBlock.typeId == Material.AIR)
						continue;

					// count adjacent non-air/non-leaf blocks
					if (leftBlock.typeId == Material.SAND || rightBlock.typeId == Material.SAND
							|| upBlock.typeId == Material.SAND || downBlock.typeId == Material.SAND
							|| aboveBlock.typeId == Material.SAND || underBlock.typeId == Material.SAND) {
	...
	private void reduceStone() {
		...
		for (int x = 1; x < snapshots.length - 1; x++) {
			for (int z = 1; z < snapshots[0][0].length - 1; z++) {
				...
				while (thisy > this.seaLevel - 1 && (this.snapshots[x][thisy][z].typeId == Material.STONE
						|| this.snapshots[x][thisy][z].typeId == Material.SANDSTONE)) {
					...
					if (leftBlock.typeId != Material.AIR && !Tag.LEAVES.isTagged(leftBlock.typeId)
							&& leftBlock.typeId != Material.VINE) {
						adjacentBlockCount++;
					}
					if (rightBlock.typeId != Material.AIR && !Tag.LEAVES.isTagged(rightBlock.typeId)
							&& rightBlock.typeId != Material.VINE) {
						adjacentBlockCount++;
					}
					if (downBlock.typeId != Material.AIR && !Tag.LEAVES.isTagged(downBlock.typeId)
							&& downBlock.typeId != Material.VINE) {
						adjacentBlockCount++;
					}
					if (upBlock.typeId != Material.AIR && !Tag.LEAVES.isTagged(upBlock.typeId)
							&& upBlock.typeId != Material.VINE) {
						adjacentBlockCount++;
					}
		...
		for (int x = 1; x < snapshots.length - 1; x++) {
			for (int z = 1; z < snapshots[0][0].length - 1; z++) {
				for (int y = this.seaLevel - 1; y < snapshots[0].length; y++) {
					...
					// skip non-logs
					if (!Tag.LOGS.isTagged(block.typeId))
						continue;

					// if in jungle biome, skip jungle logs
					if (jungleBiome && block.typeId == Material.JUNGLE_LOG)
						continue;
					...
					if (Tag.LOGS.isTagged(leftBlock.typeId) || Tag.LOGS.isTagged(rightBlock.typeId)
							|| Tag.LOGS.isTagged(upBlock.typeId) || Tag.LOGS.isTagged(downBlock.typeId)) {
		...
		for (int x = 1; x < snapshots.length - 1; x++) {
			for (int z = 1; z < snapshots[0][0].length - 1; z++) {
				for (int y = miny; y < snapshots[0].length - 1; y++) {
					...
					if (underBlock.typeId == Material.AIR || underBlock.typeId == Material.WATER
							|| Tag.LEAVES.isTagged(underBlock.typeId)) {
	...
	private void removeWallsAndTowers() {
		Material[] excludedBlocksArray = new Material[] { Material.CACTUS, Material.GRASS, Material.RED_MUSHROOM,
				Material.BROWN_MUSHROOM, Material.DEAD_BUSH, Material.DANDELION, Material.POPPY, Material.ALLIUM,
				Material.BLUE_ORCHID, Material.AZURE_BLUET, Material.RED_TULIP, Material.ORANGE_TULIP,
				Material.WHITE_TULIP, Material.PINK_TULIP, Material.OXEYE_DAISY, Material.SUGAR_CANE, Material.VINE,
				Material.PUMPKIN, Material.LILY_PAD };
	...
	private void coverSurfaceStone() {
		for (int x = 1; x < snapshots.length - 1; x++) {
			for (int z = 1; z < snapshots[0][0].length - 1; z++) {
				...
				if (block.typeId == Material.STONE || block.typeId == Material.GRAVEL
						|| block.typeId == Material.FARMLAND || block.typeId == Material.DIRT
						|| block.typeId == Material.SANDSTONE) {
			...
			for (int z = 1; z < snapshots[0][0].length - 1; z++) {
				for (int y = miny; y < snapshots[0].length - 1; y++) {
					...
					if (block.typeId == Material.WATER || block.typeId == Material.LAVA) {
						// check if block below is air or is a non-source fluid block (level 1-7 =
						// flowing, 8 = falling)
						if (underBlock.typeId == Material.AIR || (underBlock.typeId == Material.WATER
								&& (((Levelled) underBlock.data).getLevel() != 0))) {
							block.typeId = Material.AIR;
						}
					}
				}
			...
			for (int y = Math.max(this.seaLevel - 10, 0); y <= this.seaLevel; y++) {
				for (int x = 1; x < snapshots.length - 1; x++) {
					for (int z = 1; z < snapshots[0][0].length - 1; z++) {
						...
						// only consider air blocks and flowing water blocks for upgrade to water source
						// blocks
						if (block.typeId == Material.AIR
								|| (block.typeId == Material.WATER && ((Levelled) block.data).getLevel() != 0)) {
							BlockSnapshot leftBlock = this.snapshots[x + 1][y][z];
							BlockSnapshot rightBlock = this.snapshots[x - 1][y][z];
							BlockSnapshot upBlock = this.snapshots[x][y][z + 1];
							BlockSnapshot downBlock = this.snapshots[x][y][z - 1];
	...
	private void removeDumpedFluids() {
		...
		// remove any surface water or lava above sea level, presumed to be placed by
		// players
		// sometimes, this is naturally generated. but replacing it is very easy with a
		// bucket, so overall this is a good plan
	...
	private int highestY(int x, int z, boolean ignoreLeaves) {
		...
		for (y = snapshots[0].length - 1; y > 0; y--) {
			BlockSnapshot block = this.snapshots[x][y][z];
			if (block.typeId != Material.AIR && !(ignoreLeaves && block.typeId == Material.SNOW)
					&& !(ignoreLeaves && Tag.LEAVES.isTagged(block.typeId)) && !(block.typeId == Material.WATER)
	...
	static ArrayList<Material> getPlayerBlocks(Environment environment, Biome biome) {
		...
		// answer: better to leave a few player blocks than to remove too many natural
		// blocks. remember we're "restoring nature"
		// a few extra player blocks can be manually removed, but it will be impossible
		// to guess exactly which natural materials to use in manual repair of an
		// overzealous block removal
