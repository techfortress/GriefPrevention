...
class RestoreNatureProcessingTask implements Runnable 
{
	...
	private ArrayList<Material> leaves = new ArrayList<>(Arrays.asList(
			Material.OAK_LEAVES,
	    ...
	    for(int x = 1; x < snapshots.length - 1; x++)
		{
			for(int z = 1; z < snapshots[0][0].length - 1; z++)
			{
				for(int y = this.seaLevel - 1; y < snapshots[0].length; y++)
				{
					...
					if(leaves.contains(block.typeId) && (block.data & 0x4) != 0)
					{
	    ...
	    for(int x = 1; x < snapshots.length - 1; x++)
		{
			for(int z = 1; z < snapshots[0][0].length - 1; z++)
			{
				...
				while(thisy > this.seaLevel - 1 && (this.snapshots[x][thisy][z].typeId == Material.STONE || this.snapshots[x][thisy][z].typeId == Material.SANDSTONE))
				{
					...
					if(leftBlock.typeId != Material.AIR && !leaves.contains(leftBlock.typeId) && leftBlock.typeId != Material.VINE)
					{
						adjacentBlockCount++;
					}
					if(rightBlock.typeId != Material.AIR && !leaves.contains(rightBlock.typeId) && rightBlock.typeId != Material.VINE)
					{
						adjacentBlockCount++;
					}
					if(downBlock.typeId != Material.AIR && !leaves.contains(downBlock.typeId) && downBlock.typeId != Material.VINE)
					{
						adjacentBlockCount++;
					}
					if(upBlock.typeId != Material.AIR && !leaves.contains(upBlock.typeId) && upBlock.typeId != Material.VINE)
					{
						adjacentBlockCount++;
					}
	...
	private ArrayList<Material> logs = new ArrayList<>(Arrays.asList(
			Material.OAK_LOG,
			Material.SPRUCE_LOG,
			Material.BIRCH_LOG,
			Material.JUNGLE_LOG,
		...
		for(int x = 1; x < snapshots.length - 1; x++)
		{
			for(int z = 1; z < snapshots[0][0].length - 1; z++)
			{
				for(int y = this.seaLevel - 1; y < snapshots[0].length; y++)
				{
					...
					//skip non-logs
					if(!logs.contains(block.typeId)) continue;
					...
					if(logs.contains(leftBlock.typeId) || logs.contains(rightBlock.typeId) || logs.contains(upBlock.typeId) || logs.contains(downBlock.typeId))
					{
		...
		for(int x = 1; x < snapshots.length - 1; x++)
		{
			for(int z = 1; z < snapshots[0][0].length - 1; z++)
			{
				for(int y = miny; y < snapshots[0].length - 1; y++)
				{
					...
					if(underBlock.typeId == Material.AIR || underBlock.typeId == Material.WATER || underBlock.typeId == Material.LAVA || leaves.contains(underBlock.typeId))
					{
    ...
    private void fillHolesAndTrenches()
	{
		...
		notSuitableForFillBlocks.addAll(logs);
    ...
    private int highestY(int x, int z, boolean ignoreLeaves)
	{
		...
		for(y = snapshots[0].length - 1; y > 0; y--)
		{
			...
			if(block.typeId != Material.AIR &&
					...
					!(ignoreLeaves && leaves.contains(block.typeId)) &&
					!(block.typeId == Material.WATER) &&
