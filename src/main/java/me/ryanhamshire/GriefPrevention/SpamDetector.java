...
class SpamDetector {
	...
	SpamAnalysisResult AnalyzeMessage(UUID chatterID, String message, long timestamp) {
		...
		// remedy any CAPS SPAM, exception for very short messages which could be
		// emoticons like =D or XD
		...
		// if it's exactly the same as the last message from the same player and within
		// 30 seconds
		...
		// if it's very similar to the last message from the same player and within 10
		// seconds of that message
		...
		if (spam) {
			// anything above level 8 for a player which has received a warning... kick or
			// if enabled, ban
	...
	// if two strings are 75% identical, they're too close to follow each other in
	// the chat
