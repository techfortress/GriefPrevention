...
public class FlatFileDataStore extends DataStore {
	...
	void initialize() throws Exception {
		...
		// if there's no data yet, then anything written will use the schema implemented
		// by this code
		if (newDataStore) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			// all group data files start with a dollar sign. ignoring the rest, which are
			// player data files.
		...
		if (nextClaimIdFile.exists()) {
			try (FileReader fileReader = new FileReader(nextClaimIdFile.getAbsolutePath()); BufferedReader bufferedReader = new BufferedReader(fileReader)) {
				this.nextClaimID = Long.parseLong(bufferedReader.readLine());
			} catch (IOException e) {
				e.printStackTrace();
			}
		}

		// if converting up from schema version 0, rename player data files using UUIDs
		...
		if (this.getSchemaVersion() == 0) {
			...
			for (File playerFile : files) {
				...
				// if corrected casing and a record already exists using the correct casing,
				// skip this one
	...
	void loadClaimData_Legacy(File[] files) throws Exception {
		...
		for (int i = 0; i < files.length; i++) {
			if (files[i].isFile()) { // avoids folders 
				// skip any file starting with an underscore, to avoid special files not
				// representing land claims
				...
				// because some older versions used a different file name pattern before claim
				// IDs were introduced,
				...
				try {
					...
					while (line != null) {
						...
						// skip any remaining extra lines, until the "===" string, indicating the end of
						// this claim or subdivision
						line = inStream.readLine();
						...
						// otherwise there's already a top level claim, so this must be a subdivision of
						// that top level claim
						else {
				...
				// if there's any problem with the file's content, log an error message and skip
				// it
				catch (Exception e) {
					if (e.getMessage() != null && e.getMessage().contains("World not found")) {
						...
						GriefPrevention.AddLogEntry(files[i].getName() + " " + errors.toString(),
			...
			if (files[i].isFile()) // avoids folders
			{
				...
				// those files need to be "converted" by renaming them to a unique ID
					...
					File newFile = new File(
							claimDataFolderPath + File.separator + String.valueOf(this.nextClaimID) + ".yml");
					files[i].renameTo(newFile);
					files[i] = newFile;
				}
...
//    				}
					...
					// fourth line is a double-semicolon-delimited list of claims, which is
					// currently ignored
	...
	public void overrideSavePlayerData(UUID playerID, PlayerData playerData) {
		// never save data for the "administrative" account. null for claim owner ID
		// indicates administrative account
		if (playerID == null)
			return;
	...
	synchronized void migrateData(DatabaseDataStore databaseStore) {
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (!file.isFile())
				continue; // avoids folders
			if (file.isHidden())
				continue; // avoid hidden files, which are likely not created by GriefPrevention

			// all group data files start with a dollar sign. ignoring those, already
			// handled above
			if (file.getName().startsWith("$"))
				continue;
