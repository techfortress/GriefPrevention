...
public class EntityEventHandler implements Listener {
	...
	public void onLightningStrike(LightningStrikeEvent event) {
		if (event.getCause() == LightningStrikeEvent.Cause.TRIDENT)
			event.getLightning().setMetadata("GP_TRIDENT",
					new FixedMetadataValue(GriefPrevention.instance, event.getLightning().getLocation()));
	}
	...
	public void onEntityChangeBLock(EntityChangeBlockEvent event) {
		...
		// additional anti-grief steps
			...
			if (event.getTo() == Material.AIR) {
				entity.setMetadata("GP_FALLINGBLOCK",
						new FixedMetadataValue(GriefPrevention.instance, block.getLocation()));
			}
			// otherwise, the falling block is forming a block. compare new location to
			// original source
			else {
				...
				// Or if entity fell through an end portal, allow it to form, as the event is
				// erroneously fired twice in this scenario.
				if (values.size() < 1)
					return;
				...
				if (originalLocation.getBlockX() != newLocation.getBlockX()
					...
					if (GriefPrevention.instance.config_claims_worldModes
							.get(newLocation.getWorld()) == ClaimsMode.Creative) {
						event.setCancelled(true);
						return;
					}
	...
	// Used by "sand cannon" fix to ignore fallingblocks that fell through End
	// Portals
	...
	void handleExplosion(Location location, Entity entity, List<Block> blocks) {
		...
		for (int i = 0; i < blocks.size(); i++) {
			...
			if (claim != null
					&& (claim.areExplosivesAllowed || !GriefPrevention.instance.config_blockClaimExplosions)) {
	...
	public void onEntitySpawn(CreatureSpawnEvent event) {
		...
		// otherwise, just apply the limit on total entities per claim (and no spawning
		// in the wilderness!)
	...
	public void onEntityCombustByEntity(EntityCombustByEntityEvent event) {
		// handle it just like we would an entity damge by entity event, except don't
		// send player messages to avoid double messages
		// in cases like attacking with a flame sword or flame arrow, which would ALSO
		// trigger the direct damage event handler
	...
	private void handleEntityDamageEvent(EntityDamageEvent event, boolean sendErrorMessagesToPlayers) {
		...
		// protect pets from environmental damage types which could be easily caused by
		// griefers
		if (event.getEntity() instanceof Tameable
			...
			if (tameable.isTamed()) {
				...
				if (cause != null && (cause == DamageCause.ENTITY_EXPLOSION || cause == DamageCause.FALLING_BLOCK
						|| cause == DamageCause.FIRE || cause == DamageCause.FIRE_TICK || cause == DamageCause.LAVA
						|| cause == DamageCause.SUFFOCATION || cause == DamageCause.CONTACT
		...
		if (damageSource != null) {
			...
			if (damageSource.getType() == EntityType.AREA_EFFECT_CLOUD && event.getEntityType() == EntityType.PLAYER
					&& GriefPrevention.instance.pvpRulesApply(event.getEntity().getWorld())) {
				...
				// case 2: in a pvp safe zone
					...
					Claim damagedClaim = GriefPrevention.instance.dataStore.getClaimAt(damaged.getLocation(), false,
							damagedData.lastClaim);
		...
		// if the attacker is a firework from a crossbow by a player and defender is a
		// player (nonpvp)
		if (firework != null && event.getEntityType() == EntityType.PLAYER
				&& !GriefPrevention.instance.pvpRulesApply(attacker.getWorld())) {
		...
		if (attacker != null && event.getEntityType() == EntityType.PLAYER
				&& GriefPrevention.instance.pvpRulesApply(attacker.getWorld())) {
			// FEATURE: prevent pvp in the first minute after spawn, and prevent pvp when
			// one or both players have no inventory
		...
		if (event instanceof EntityDamageByEntityEvent) {
			...
			// protect players from being attacked by other players' pets when protected
			// from pvp
			if (event.getEntityType() == EntityType.PLAYER) {
				Player defender = (Player) event.getEntity();
				...
				if (damager != null && damager instanceof Tameable) {
					...
					if (pet.isTamed() && pet.getOwner() != null) {
						// if defender is NOT in pvp combat and not immune to pvp right now due to
						// recent respawn
						PlayerData defenderData = GriefPrevention.instance.dataStore
								.getPlayerData(event.getEntity().getUniqueId());
			...
			if (subEvent.getEntityType() == EntityType.ITEM_FRAME || subEvent.getEntityType() == EntityType.ARMOR_STAND
				...
				if (subEvent.getEntityType() == EntityType.VILLAGER
				...
				if (claim != null) {
					...
					if (attacker == null) {
						// exception case
						if (event.getEntityType() == EntityType.VILLAGER && damageSource != null
								&& damageSource.getType() == EntityType.ZOMBIE) {
							return;
						}
			...
			// if the entity is an non-monster creature (remember monsters disqualified
			// above), or a vehicle
			if (((subEvent.getEntity() instanceof Creature || subEvent.getEntity() instanceof WaterMob)
				...
				if (subEvent.getEntity() instanceof Tameable) {
					...
					if (tameable.isTamed() && tameable.getOwner() != null) {
						...
						if (attacker != null) {
							...
							// otherwise disallow in non-pvp worlds (and also pvp worlds if configured to do
							// so)
							if (!GriefPrevention.instance.pvpRulesApply(subEvent.getEntity().getLocation().getWorld())
									|| (GriefPrevention.instance.config_pvp_protectPets
											&& subEvent.getEntityType() != EntityType.WOLF)) {
								OfflinePlayer owner = GriefPrevention.instance.getServer().getOfflinePlayer(ownerID);
								String ownerName = owner.getName();
								if (ownerName == null)
									ownerName = "someone";
								String message = GriefPrevention.instance.dataStore
										.getMessage(Messages.NoDamageClaimedEntity, ownerName);
								if (attacker.hasPermission("griefprevention.ignoreclaims"))
									message += "  " + GriefPrevention.instance.dataStore
											.getMessage(Messages.IgnoreClaimsAdvertisement);
								if (sendErrorMessagesToPlayers)
									GriefPrevention.sendMessage(attacker, TextMode.Err, message);
								PreventPvPEvent pvpEvent = new PreventPvPEvent(new Claim(
										subEvent.getEntity().getLocation(), subEvent.getEntity().getLocation(), null,
							...
							// disallow players attacking tamed wolves (dogs) unless under attack by said
							// wolf
							else if (tameable.getType() == EntityType.WOLF) {
								if (!tameable.getOwner().equals(attacker)) {
									if (((Wolf) tameable).getTarget() != null) {
										if (((Wolf) tameable).getTarget() == attacker)
											return;
									}
									event.setCancelled(true);
									String ownerName = GriefPrevention.instance.getServer().getOfflinePlayer(ownerID)
											.getName();
									String message = GriefPrevention.instance.dataStore
											.getMessage(Messages.NoDamageClaimedEntity, ownerName);
									if (attacker.hasPermission("griefprevention.ignoreclaims"))
				...
				if (attacker == null && damageSource != null && damageSource.getType() != EntityType.CREEPER
						...
						&& damageSource.getType() != EntityType.WITCH && !(damageSource instanceof Projectile)
						&& !(damageSource instanceof Explosive) && !(damageSource instanceof ExplosiveMinecart)) {
					return;
				}
				...
				// if it's claimed
				if (claim != null) {
					// if damaged by anything other than a player (exception villagers injured by
					// zombies in admin claims), cancel the event
					...
					if (attacker == null) {
						...
						if (event.getEntityType() == EntityType.VILLAGER && damageSource != null
								&& (damageSource.getType() == EntityType.ZOMBIE
										|| damageSource.getType() == EntityType.VINDICATOR
										|| damageSource.getType() == EntityType.EVOKER
										|| damageSource.getType() == EntityType.EVOKER_FANGS
										|| damageSource.getType() == EntityType.VEX)) {
							return;
						}
					...
					// otherwise the player damaging the entity must have permission, unless it's a
					// dog in a pvp world
					else if (!(event.getEntity().getWorld().getPVP()
						...
						if (noContainersReason != null) {
							...
							// //RoboMWM: except for tridents
							if (arrow != null && arrow.getType() != EntityType.TRIDENT)
								arrow.remove();
							if (damageSource != null && damageSource.getType() == EntityType.FIREWORK
	...
	public void onCrossbowFireWork(EntityShootBowEvent shootEvent) {
		if (shootEvent.getEntity() instanceof Player && shootEvent.getProjectile() instanceof Firework) {
			shootEvent.getProjectile().setMetadata("GP_FIREWORK",
					new FixedMetadataValue(GriefPrevention.instance, shootEvent.getEntity()));
		}
	}
	...
	public void onVehicleDamage(VehicleDamageEvent event) {
		...
		if (claim != null) {
			...
			// otherwise the player damaging the entity must have permission
				...
				if (noContainersReason != null) {
					event.setCancelled(true);
					String message = GriefPrevention.instance.dataStore.getMessage(Messages.NoDamageClaimedEntity,
							claim.getOwnerName());
					if (attacker.hasPermission("griefprevention.ignoreclaims"))
						message += "  "
	...
	public void onPotionSplash(PotionSplashEvent event) {
		...
		for (PotionEffect effect : effects) {
			...
			// restrict some potions on claimed animals (griefers could use this to kill or
			// steal animals over fences) //RoboMWM: include villagers
			...
			for (LivingEntity effected : event.getAffectedEntities()) {
				...
				// it's disabled)
					...
					Claim attackerClaim = this.dataStore.getClaimAt(thrower.getLocation(), false,
							attackerData.lastClaim);
					...
					Claim defenderClaim = this.dataStore.getClaimAt(effectedPlayer.getLocation(), false,
							defenderData.lastClaim);
	...
	public static final HashSet<PotionEffectType> positiveEffects = new HashSet<PotionEffectType>(
			Arrays.asList(PotionEffectType.ABSORPTION, PotionEffectType.DAMAGE_RESISTANCE,
					PotionEffectType.FAST_DIGGING, PotionEffectType.FIRE_RESISTANCE, PotionEffectType.HEAL,
					PotionEffectType.HEALTH_BOOST, PotionEffectType.INCREASE_DAMAGE, PotionEffectType.INVISIBILITY,
					PotionEffectType.JUMP, PotionEffectType.NIGHT_VISION, PotionEffectType.REGENERATION,
