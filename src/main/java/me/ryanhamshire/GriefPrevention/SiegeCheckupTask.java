...
class SiegeCheckupTask implements Runnable {
	...
	public void run() {
		...
		// if this is a new claim and he has some permission there, extend the siege to
		// include it
		...
		// otherwise they both left and aren't close to each other, so call the attacker
		// the winner (defender escaped, possibly after a chase)
		else {
			dataStore.endSiege(this.siegeData, attacker.getName(), defender.getName(), null);
		}
	}
