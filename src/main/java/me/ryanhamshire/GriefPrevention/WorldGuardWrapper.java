...
class WorldGuardWrapper {
	...
	public WorldGuardWrapper() throws ClassNotFoundException {
		this.worldGuard = (WorldGuardPlugin) GriefPrevention.instance.getServer().getPluginManager()
	...
	public boolean canBuild(Location lesserCorner, Location greaterCorner, Player creatingPlayer) {
		try {
			...
			if (manager != null) {
				ProtectedCuboidRegion tempRegion = new ProtectedCuboidRegion("GP_TEMP",
				...
				for (ProtectedRegion region : overlaps.getRegions()) {
					if (!manager.getApplicableRegions(region).testState(localPlayer, Flags.BUILD)) {
						return false;
					}
				}
				return true;
