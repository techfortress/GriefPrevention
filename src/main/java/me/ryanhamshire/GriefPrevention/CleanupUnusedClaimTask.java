...
class CleanupUnusedClaimTask implements Runnable {
	...
	public void run() {
		...
		if (claim.getArea() <= areaOfDefaultClaim && GriefPrevention.instance.config_claims_chestClaimExpirationDays > 0) {
			// if the owner has been gone at least a week, and if he has ONLY the new player
		...
		// if configured to always remove claims after some inactivity period without
		// exceptions...
