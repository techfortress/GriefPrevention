...
public class Claim {
	...
	// exclusionClaim is another claim indicating an sub-area to be excluded from
	// this operation
	// it may be null
	...
	public boolean isNear(Location location, int howNear) {
		Claim claim = new Claim(
				...
				null, new ArrayList<String>(), new ArrayList<String>(), new ArrayList<String>(),
				new ArrayList<String>(), null);

		return claim.contains(location, false, true);
	}

	// permissions. note administrative "public" claims have different rules than
	// other claims
	...
	public String allowEdit(Player player) {
		// if we don't know who's asking, always say no (i've been told some mods can
		...
		def: {
			...
			if (this.parent != null) {
				...
				if (!inheritNothing) {
					message = this.parent.allowEdit(player);
					break def;
				}
			}
	...
	private List<Material> placeableFarmingBlocksList = Arrays.asList(Material.PUMPKIN_STEM, Material.WHEAT,
	...
	public String allowBuild(Player player, Material material) {
		...
		// when a player tries to build in a claim, if he's under siege, the siege may
		// extend to include the new claim
		...
		def: {
			...
			if (this.parent != null) {
				...
				if (!inheritNothing) {
					message = this.parent.allowBuild(player, material);
					break def;
				}
			}
	...
	public String allowAccess(Player player) {
		...
		// default system before event firing
		def: {
			// following a siege where the defender lost, the claim will allow everyone
			// access for a time
			if (this.doorsOpen) {
				message = null;
				break def;
			...
			if (this.parent != null) {
				...
				if (!inheritNothing) {
					message = this.parent.allowAccess(player);
					break def;
				}
			}
	...
	public String allowContainers(Player player) {
		...
		// trying to access inventory in a claim may extend an existing siege to include
		// this claim
		GriefPrevention.instance.dataStore.tryExtendSiege(player, this);
		...
		def: {
			...
			if (this.parent != null) {
				...
				if (!inheritNothing) {
					message = this.parent.allowContainers(player);
					break def;
				}
			}
	
			// error message for all other cases
			message = GriefPrevention.instance.dataStore.getMessage(Messages.NoContainersPermission,
					this.getOwnerName());
			if (player.hasPermission("griefprevention.ignoreclaims"))
				message += "  " + GriefPrevention.instance.dataStore.getMessage(Messages.IgnoreClaimsAdvertisement);
		}
	...
	public String allowGrantPermission(Player player) {
		...
		if (this.allowEdit(player) == null)
			return null; // the event do not consider this to avoid loop
		...
		def: {
			...
			if (this.parent != null) {
				...
				if (!inheritNothing) {
					message = this.parent.allowGrantPermission(player);
					break def;
				}
			}
	...
	public void getPermissions(ArrayList<String> builders, ArrayList<String> containers, ArrayList<String> accessors,
		...
		Iterator<Map.Entry<String, ClaimPermission>> mappingsIterator = this.playerIDToClaimPermissionMap.entrySet()
				.iterator();
	...
	public boolean contains(Location location, boolean ignoreHeight, boolean excludeSubdivisions) {
		...
		boolean inClaim = (ignoreHeight || y >= this.lesserBoundaryCorner.getY())
				&& x >= this.lesserBoundaryCorner.getX() && x < this.greaterBoundaryCorner.getX() + 1
				&& z >= this.lesserBoundaryCorner.getZ() && z < this.greaterBoundaryCorner.getZ() + 1;
	...
	boolean overlaps(Claim otherClaim) {
		// NOTE: if trying to understand this makes your head hurt, don't feel bad - it
		// hurts mine too.
		...
		if (otherClaim.contains(this.greaterBoundaryCorner, true, false))
			return true;
		if (otherClaim.contains(new Location(this.lesserBoundaryCorner.getWorld(),
				this.lesserBoundaryCorner.getBlockX(), 0, this.greaterBoundaryCorner.getBlockZ()), true, false))
			return true;
		if (otherClaim.contains(new Location(this.lesserBoundaryCorner.getWorld(),
				this.greaterBoundaryCorner.getBlockX(), 0, this.lesserBoundaryCorner.getBlockZ()), true, false))
			return true;

		// verify that no claim's lesser boundary point is inside this new claim, to
		// cover the "existing claim is entirely inside new claim" case
		if (this.contains(otherClaim.getLesserBoundaryCorner(), true, false))
			return true;

		// verify this claim doesn't band across an existing claim, either horizontally
		// or vertically
		...
		for (Chunk chunk : chunks) {
			...
			for (int i = 0; i < entities.length; i++) {
				...
				if (!(entity instanceof Player) && this.contains(entity.getLocation(), false, false)) {
					totalEntities++;
					if (remove && totalEntities > maxEntities)
						entity.remove();
				}
			}
		}
	...
	long getPlayerInvestmentScore() {
		...
		ArrayList<Material> playerBlocks = RestoreNatureProcessingTask.getPlayerBlocks(
