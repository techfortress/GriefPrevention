...
public class Claim {
	...
	private boolean hasExplicitPermission(Player player, ClaimPermission level) {
		...
		while (iterator.hasNext()) {
			...
			if (playerID.equalsIgnoreCase(identifier) && this.playerIDToClaimPermissionMap.get(identifier) == level)
				...
				// check permission
				if (player.hasPermission(permissionIdentifier)
						&& this.playerIDToClaimPermissionMap.get(identifier) == level)
					return true;
			}
		}
	...
	public String allowAccess(Player player) {
		...
		if (ClaimPermission.Build == permissionLevel || ClaimPermission.Inventory == permissionLevel
				|| ClaimPermission.Access == permissionLevel)
			return null;
	...
	public String allowContainers(Player player) {
		...
		if (this.siegeData != null) {
			return GriefPrevention.instance.dataStore.getMessage(Messages.NoContainersSiege,
					siegeData.attacker.getName());
		}
		...
		String reason = GriefPrevention.instance.dataStore.getMessage(Messages.NoContainersPermission,
				this.getOwnerName());
		if (player.hasPermission("griefprevention.ignoreclaims"))
			reason += "  " + GriefPrevention.instance.dataStore.getMessage(Messages.IgnoreClaimsAdvertisement);
		return reason;
