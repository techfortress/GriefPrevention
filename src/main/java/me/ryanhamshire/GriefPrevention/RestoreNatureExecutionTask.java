...
class RestoreNatureExecutionTask implements Runnable {
	...
	// player who should be notified about the result (will see a visualization when
	// the restoration is complete)
	private Player player;

	public RestoreNatureExecutionTask(BlockSnapshot[][][] snapshots, int miny, Location lesserCorner,
			Location greaterCorner, Player player) {
	...
	public void run() {
		...
		// note that the edge of the results is not applied (the 1-block-wide band
		// around the outside of the chunk)
		// those data were sent to the processing thread for referernce purposes, but
		// aren't part of the area selected for restoration
		...
		for (int i = 0; i < entities.length; i++) {
			...
			if (!(entity instanceof Player || entity instanceof Animals)) {
				// hanging entities (paintings, item frames) are protected when they're in land
				// claims
