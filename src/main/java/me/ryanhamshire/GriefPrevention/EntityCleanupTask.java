...
class EntityCleanupTask implements Runnable {
	...
	public void run() {
		...
		for (int i = 0; i < worlds.size(); i++) {
			...
			// starting and stopping point. each execution of the task scans 10% of the
			// server's (loaded) entities
			...
			for (; j < entities.size() && j < k; j++) {
				...
				if (entity instanceof Boat) { // boats must be occupied
					...
					if (vehicle.getVelocity().lengthSquared() != 0) {
						if (vehicle.isEmpty() || !vehicle.getPassengers().stream().anyMatch(passager -> passager.getType() == EntityType.PLAYER)) {
							remove = true;
						}
					}
		...
		// starting and stopping point. each execution of the task scans 5% of the
		// server's claims
		...
		// schedule the next run of this task, in 3 minutes (20L is approximately 1
		// second)
